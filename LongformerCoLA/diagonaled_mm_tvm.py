...
class DiagonaledMM(torch.autograd.Function):
    ...
    def _compile_function(dtype = torch.int32, device= 'cpu'):
        b, n, h, t1d3 = 4, 512, 12, 3072 
        _, _, _, m = 4, 4, 16, 3072  

        X = torch.randn(b, n, h, t1d3)
        Y = torch.randn(b, n, h, m)
        D = torch.randint(0, 5, (h,), dtype=torch.int32, device=device)  # Assuming D is within range [0, 4]

        Z = torch.zeros(b, n, h, m)
        ...
        for l in range(b):
            for i in range(n):
                for q in range(h):
                    for k in range(t1d3):
                        if t1d3 == m:
                            if i + D[q] * (k - w) >= 0 and i + D[q] * (k - w) < n:
                                Z[l, i, q, k] = X[l, i, q, k] * Y[l, i + D[q] * (k - w), q, k]
                            else:
                                Z[l, i, q, k] = padding if padding is not None else 0
                        else:
                            if i + D[q] * (k - w) >= 0 and i + D[q] * (k - w) < n:
                                Z[l, i, q, k] = X[l, i, q, k] * Y[l, i + D[q] * (k - w), q, k]
                            else:
    ...
    def _get_lib_filename(dtype: str, device: str):
        base_filename = 'lib/lib_diagonaled_mm'
    ...
    def _load_compiled_function(dtype: str, device: str):
        ...
        for potential_dir in potential_dirs:
            ...
            if os.path.isfile(filepath):
                print('Loading binary from: {}'.format(filepath))
                with open(filepath, 'rb') as f:
                    return pickle.load(f)  # Assuming the binary file is pickled
        return None
    @staticmethod
    def _get_function(dtype: str, device: str):
        ...
        if args not in DiagonaledMM.function_dict:
            ...
            diagonaled_mm_pytorch = DiagonaledMM.function_wrapper(diagonaled_mm)  # wrap it as a pytorch function
    ...
    @staticmethod
    def function_wrapper(input_tensor):
        input_array = input_tensor.numpy()
        output_array = DiagonaledMM.diagonaled_mm(input_array)
