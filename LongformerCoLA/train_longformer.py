...
from model import build_real_longformer
...
def train_model(config):
    ...
    for epoch in range(init_epoch, config['epochs']):
        ...
        for batch in batch_iterator:
            ...
            classification_output = model.forward(encoder_input)

            output = threshold(classification_output, 0.5)

            # outputs = torch.argmax(classification_output, dim = -1)
            # print()
            print(output, label)
            label = label.float()
